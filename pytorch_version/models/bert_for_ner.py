    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                active_logits = tools.mps_utils.mask_select_on_cpu(
                    input=logits.reshape(-1, self.num_labels),
                    mask=active_loss,
                )
                active_labels = tools.mps_utils.mask_select_on_cpu(
                    input=labels.reshape(-1),
                    mask=active_loss,
                )
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        if start_positions is not None and self.training:
            ...
            if not self.soft_label:
                label_logits = torch.argmax(
                    label_logits, -1).unsqueeze(2).float()
