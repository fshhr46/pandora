...
from processors.cls_sentence import batch_collate_fn
...
def predict_pretrained_model(activation_type, arg_list):
    args, config, tokenizer, model, _ = setup(arg_list=arg_list)
    prefix = os.path.join("predict", activation_type)
    os.makedirs(os.path.join(args.output_dir, prefix), exist_ok=True)
    predict(args, model, tokenizer, prefix=prefix)


def evaluate_pretrained_model(activation_type, arg_list):
    args, config, tokenizer, model, _ = setup(arg_list=arg_list)
    prefix = os.path.join("evaluate", activation_type)
    os.makedirs(os.path.join(args.output_dir, prefix), exist_ok=True)
...
def main():
    ...
    train_eval_test(activation_type, arg_list)
    # predict_pretrained_model(activation_type, arg_list)
    # evaluate_pretrained_model(activation_type, arg_list)


def train_eval_test(activation_type, arg_list):
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            prefix = checkpoint.split(
                '/')[-1] if checkpoint.find('checkpoint') != -1 else ""
            prefix = os.path.join("predict", activation_type, prefix)
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            os.makedirs(os.path.join(args.output_dir, prefix), exist_ok=True)
            predict(args, model, tokenizer, prefix=prefix)
