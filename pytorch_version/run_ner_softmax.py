...
def predict(args, model, tokenizer, prefix="", data_type="test"):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0],
                      "attention_mask": batch[1], "labels": None}
            if args.model_type != "distilbert":
                # XLM and RoBERTa don"t use segment_ids
                inputs["token_type_ids"] = (batch[2] if args.model_type in [
                                            "bert", "xlnet"] else None)
    ...
    test_text = []
    with open(os.path.join(args.data_dir, f"{data_type}.json"), 'r') as fr:
        count = 0
        for line in fr:
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask,
                            all_segment_ids, all_lens, all_label_ids)
    return dataset
