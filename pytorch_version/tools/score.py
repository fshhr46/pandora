#!/usr/bin/python
# coding:utf8
"""
@author: Cong Yu
@time: 2020-01-09 22:53
"""
import json
import os
from pathlib import Path


def main():
    base_dir = str(Path(os.path.dirname(__file__)).parent.absolute())
    predict_file = os.path.join(base_dir,
                                "outputs/bert-base-chinese/cluener_output/bert/predict/softmax/eval_2_output/test_submit.json")
    truth_file = os.path.join(base_dir, "CLUEdatasets/cluener/dev.json")
    f_score, avg = get_f1_score(
        predict_file=predict_file, truth_file=truth_file)


def get_f1_score_label(pre_lines, gold_lines, label="organization"):
    """
    打分函数
    """
    # pre_lines = [json.loads(line.strip()) for line in open(pre_file) if line.strip()]
    # gold_lines = [json.loads(line.strip()) for line in open(gold_file) if line.strip()]
    print(f"==========\tLabel: {label}")
    TP = 0
    FP = 0
    FN = 0
    for pre, gold in zip(pre_lines, gold_lines):
        pre = pre["label"].get(label, {}).keys()
        gold = gold["label"].get(label, {}).keys()
        for i in pre:
            if i in gold:
                TP += 1
            else:
                FP += 1
        for i in gold:
            if i not in pre:
                FN += 1
    print(TP, FP, FN)
    p = TP / (TP + FP)
    r = TP / (TP + FN)
    f = 2 * p * r / (p + r)
    print(p, r, f)
    return f


def get_f1_score(predict_file="ner_predict.json", truth_file="data/thuctc_valid.json"):
    pre_lines = [json.loads(line.strip())
                 for line in open(predict_file) if line.strip()]
    gold_lines = [json.loads(line.strip())
                  for line in open(truth_file) if line.strip()]
    f_score = {}
    labels = ['address', 'book', 'company', 'game', 'government',
              'movie', 'name', 'organization', 'position', 'scene']
    sum = 0
    for label in labels:
        f = get_f1_score_label(pre_lines, gold_lines, label=label)
        f_score[label] = f
        sum += f
    avg = sum / len(labels)

    print("========== Summary ==========")
    print(f"\nf_score: \n{json.dumps(f_score, indent=4)}")
    print(f"\naverage: \n{json.dumps(avg, indent=4)}")
    return f_score, avg


if __name__ == "__main__":
    main()
