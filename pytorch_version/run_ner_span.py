...
from processors.ner_span import batch_collate_fn
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for step, f in enumerate(eval_features):
        ...
        input_ids = torch.tensor(
            [f.input_ids[:input_lens]], dtype=torch.long).to(args.device)
        input_mask = torch.tensor(
            [f.input_mask[:input_lens]], dtype=torch.long).to(args.device)
        segment_ids = torch.tensor(
            [f.segment_ids[:input_lens]], dtype=torch.long).to(args.device)
        start_ids = torch.tensor(
            [f.start_ids[:input_lens]], dtype=torch.long).to(args.device)
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                # XLM and RoBERTa don"t use segment_ids
                inputs["token_type_ids"] = (segment_ids if args.model_type in [
                                            "bert", "xlnet"] else None)
            outputs = model(**inputs)
...
def predict(args, model, tokenizer, prefix=""):
    ...
    test_dataloader = DataLoader(
        test_dataset, sampler=test_sampler, batch_size=1, collate_fn=batch_collate_fn)
    # Eval!
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        pbar(step)
    print(" ")
    output_predic_file = os.path.join(
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask,
                            all_segment_ids, all_start_ids, all_end_ids, all_input_lens)
    return dataset
