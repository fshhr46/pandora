...
def build_train_report(
    ...
    for example, pred in zip(examples, predictions):
        ...
        json_d['pred'] = [pred_class]

        # DOC related attributes
        # TODO: Support multi-labels
        labeled_class = example.labels[0]
        unseen_class = labeled_class not in label_list
        json_d['rejected'] = pred['rejected']
        json_d['unseen_class'] = unseen_class
        pre_lines.append(json_d)

    json_to_text(output_submit_file, pre_lines)
    report.build_report(pre_lines=pre_lines,
                        truth_lines=pre_lines,
                        label_list=label_list,
                        build_doc_report=use_doc,
...
def get_data_processor(
        ...
        resource_dir: str,
        num_folds: int,
        doc_hold_out: float) -> SentenceProcessor:
...
def load_and_cache_examples(args,
    ...
    examples = processor.load_examples(args.data_dir,
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            feat = processor.convert_example_to_feature(
                example,
                log_data=ex_index < 5,
